document.addEventListener('DOMContentLoaded', () => {
  // helper to set innerHtml safely (we trust our hidden HTML here)
  ...
  // toggle heading/list pair visibility based on list content
  function toggleListAndHeading(listEl, headingEl) {
    if (!listEl) return;
    const hasContent = (listEl.innerHTML || "").trim() !== "";
    show(listEl, hasContent);
    show(headingEl, hasContent);
  }

  // main open logic given a course key and optional sourceCard for title
  function openCourseModalByKey(courseKey, sourceCard) {
    if (!courseKey) return;
    const source = document.getElementById('course-' + courseKey);

    // clear defaults
    setInner('col-left-ug', '');
    setInner('col-left-int', '');
    setInner('col-right-pg', '');
    setInner('col-right-phd', '');
    setInner('col-right-diploma', '');
    setInner('col-top-programs', '');
    document.getElementById('col-right-diploma-wrap').style.display = 'none';
    document.getElementById('col-top-programs-wrap').style.display = 'none';
    ...
    // Title: prefer visible card title, else data-title from source
    const cardTitle = sourceCard?.querySelector('.card-title')?.innerText?.trim();
    document.getElementById('modalCourseName').textContent = cardTitle || source.dataset.title || 'Programs';

    // copy lists instantly
    const ug = source.querySelector('.ug')?.innerHTML || "";
    const integrated = source.querySelector('.integrated')?.innerHTML || "";
    const pg = source.querySelector('.pg')?.innerHTML || "";
    const phd = source.querySelector('.phd')?.innerHTML || "";
    const diploma = source.querySelector('.diploma')?.innerHTML || "";
    const topPrograms = source.querySelector('.topPrograms')?.innerHTML || "";

    setInner('col-left-ug', ug);
    setInner('col-left-int', integrated);
    setInner('col-right-pg', pg);
    setInner('col-right-phd', phd);

    if (diploma.trim()) {
      setInner('col-right-diploma', diploma);
      document.getElementById('col-right-diploma-wrap').style.display = '';
    } else {
      document.getElementById('col-right-diploma-wrap').style.display = 'none';
    }

    if (topPrograms.trim()) {
      setInner('col-top-programs', topPrograms);
      document.getElementById('col-top-programs-wrap').style.display = '';
    } else {
      document.getElementById('col-top-programs-wrap').style.display = 'none';
    }

    // === now handle heading visibility ===
    // pick left & right columns in the modal
    const modalBody = document.querySelector('#courseModal .modal-body');
    const cols = modalBody?.querySelectorAll('.col-lg-6') || [];
    const leftCol = cols[0] || null;
    const rightCol = cols[1] || null;

    // left col headings: [0] -> Undergraduate, [1] -> Integrated
    const leftH6s = leftCol ? leftCol.querySelectorAll('h6') : [];
    const leftUgHeading = leftH6s[0] || null;
    const leftIntHeading = leftH6s[1] || null;
    const leftUgList = document.getElementById('col-left-ug');
    const leftIntList = document.getElementById('col-left-int');

    // right col headings: [0] -> Postgraduate, [1] -> Ph.D.
    const rightH6s = rightCol ? rightCol.querySelectorAll('h6') : [];
    const rightPgHeading = rightH6s[0] || null;
    const rightPhdHeading = rightH6s[1] || null;
    const rightPgList = document.getElementById('col-right-pg');
    const rightPhdList = document.getElementById('col-right-phd');

    // toggle each pair
    toggleListAndHeading(leftUgList, leftUgHeading);
    toggleListAndHeading(leftIntList, leftIntHeading);
    toggleListAndHeading(rightPgList, rightPgHeading);
    toggleListAndHeading(rightPhdList, rightPhdHeading);

    // diploma and top-programs are wrapped; the wrap's display already toggled above,
    // so no extra heading toggling is required (their <h6> sits inside the wrap).
    // show the modal
    new bootstrap.Modal(document.getElementById('courseModal')).show();
  }

  // attach click + key handlers to whole card elements
  document.querySelectorAll('.course-card[tabindex]').forEach(card => {
    // click: open modal
    card.addEventListener('click', (ev) => {
      // if user clicked an inner interactive element (like a link or button), allow its default
      const interactive = ev.target.closest('a, button, input, textarea, select');
      if (interactive && interactive !== card) return;
      const key = card.dataset.course || card.querySelector('.openCourseModal')?.dataset.course;
      openCourseModalByKey(key, card);
    });
  ...
  // ensure legacy/specific CTA images also work (if someone clicks the arrow image)
  document.querySelectorAll('.openCourseModal').forEach(cta => {
    cta.addEventListener('click', (ev) => {
      ev.stopPropagation(); // prevent parent's click from double-firing
      const key = cta.dataset.course;
      // find nearest card to prefer its visible title
...
(function(){
 ...
 (function () {
    ...
    const RESUME_AFTER_MANUAL = 4000; // resume auto-slide this many ms after manual action
